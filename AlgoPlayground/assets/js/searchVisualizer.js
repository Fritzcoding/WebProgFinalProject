/* SearchVisualizer — binary search with visuals */
(function(window){
  class SearchVisualizer {
    ...
    _apply(highlights = {}){
      ...
      this.array.forEach((v,i)=>{
        ...
        el.textContent = this.array.length <= 40 ? v : '';
      });
    }
    ...
    async step(){
      ...
      // else not supported: user should run to generate steps
    }
    ...
    _waitResume(){ return new Promise(r=>{ const i = setInterval(()=>{ if(!this.paused){ clearInterval(i); r(); } }, 50); }); }
  }
